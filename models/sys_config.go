...
type SysConfig struct {
	...
	Content string `orm:"column(content);size(512);null"`
...
// GetAllSysConfig retrieves all SysConfig matches certain condition. Returns empty list if
// no records exist
func GetAllSysConfig(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) ([]interface{}, error) {
	var err error
	ml := make([]interface{}, 0)
	o := orm.NewOrm()
	qs := o.QueryTable(new(SysConfig))
	// query k=v
	for k, v := range query {
